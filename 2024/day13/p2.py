def parse(s, add=False):
    return tuple(int(q[2:]) + (10000000000000 if add else 0) for q in s.split(", "))

with open("input.txt") as file:
    machines = file.read().split("\n\n")
    machines = [machine.split("\n") for machine in machines]
    machines = [(parse(a[10:]),parse(b[10:]),parse(prize[7:],add=True)) for [a,b,prize] in machines]
...
for a, b, prize in machines:
    ...
    if (determinant == 0):
        ...
        if not (x < 0 or y < 0 or x_remainder != 0 or y_remainder != 0):
            # print(x, y)
            total += x*3 + y
