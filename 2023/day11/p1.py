...
with open("input.txt") as file:
    ...
    for (x1, y1), (x2, y2) in combinations(galaxy_locations, 2):
        if x1 >= x2:
            x_dist = blank_columns[x2:x1].count(True) - (x2 - x1)
        else:
            x_dist = blank_columns[x1:x2].count(True) - (x1 - x2)
        if y1 >= y2:
            y_dist = blank_rows[y2:y1].count(True) - (y2 - y1)
        else:
            y_dist = blank_rows[y1:y2].count(True) - (y1 - y2)
        sum += x_dist + y_dist
    print(sum)
