...
def print_path(predecessors, width, height):
    passed_through = [[False for i in range(width)] for j in range(height)]
    passed_through[height-1][width-1] = True
    current = (width-1, height-1, final_dir, final_mult)
    while (current[0], current[1]) != (0,0):
        current = predecessors[current]
        passed_through[current[1]][current[0]] = True
...
with open("input.txt") as file:
    ...
    while not queue.empty():
        ...
        if x == width-1 and y == height-1 and multiplicity >= 4:
            final_dir, final_mult = direction, multiplicity
            print(distance)
            break
        for x2, y2 in get_neighbours(x,y,direction, multiplicity, width, height):
            ...
            if (new_direction, new_multiplicity) not in visited[y2][x2]:
                ...
                predecessors[(x2,y2, new_direction, new_multiplicity)] = (x,y,direction, multiplicity)
    #print_path(predecessors, width, height)        